...
`.pj-weakness-h`（「しかし、／ファージには弱点がある」）は デザイン上 2行で 111px を想定しているが、
日本語グリフの行送りで実際は **119px** になり、`top: 3258` の本文に 8px 食い込んでいた。

**修正**: `.pj-weakness-b` の `top` を `3258` → `3272`（他セクションと同じ 8px の間隔）。

> [!NOTE]
> 同種の「デザインの想定行高より実測が高い」ずれは、`.pj-but` と `.pj-resistance-h` の間にも
...
- テキスト同士の実質的な衝突なし。
  残る箱の重なり（`phage-h` × `img-what`、`how-b` × `img-how`）は デザイン由来の余白で、
...
| `/ja/project/` (JA) | デザインカンプ由来のストーリーページ | ページ内に直書き |
...
デザインの座標は日本語の文字数に合わせて決まっているため、**実ブラウザで折り返しを計測しながら**
1行の長さを詰めた。
...
- `.pj-goal` のフォントは日本語版のみ Sawarabi Gothic（デザインの指定）。
...
- 複数行の項目は、**改行した位置がそのまま画面上の改行になる**
  （ページ側で `
` 区切りを `<br>` に変換）
- レイアウトが デザイン座標の絶対配置のため、**行数を増やしすぎると図形や画像に重なる**。
