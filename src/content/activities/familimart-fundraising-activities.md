---
title: FamilyMart Fundraising activities
description: We have partnered with FamilyMart to place donation boxes in
  selected stores to support our project. This fundraising activity generated
  9,346 yen + 2 Euro in donations to support our project.
title_ja: ファミリーマートにおける募金活動
description_ja: |
  私たちは、プロジェクトへのご支援を募るため、ファミリーマートと連携し、一部店舗に募金箱を設置しました。

  この募金活動を通じて、9,346円および2ユーロのご寄付をいただきました。いただいたご寄付は、私たちのプロジェクトの研究・活動費として大切に活用させていただきます。
image: /media/IMG_4225 (2)-1.webp
imagePlaceholder: FamilyMart
imagePlaceholder_ja: ファミリーマート
---
